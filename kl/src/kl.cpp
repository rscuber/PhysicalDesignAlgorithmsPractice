...
inline int KL::Graph::DetlaGain(int a, int b) const {
...
std::pair<int, std::pair<int, int>> KL::Graph::MaxGain() const {
    ...
    for (int i = 0; i < this->vertex_num(); i++) {
        for (int j = i + 1; j < this->vertex_num(); j++) {
            ...
            int cur_detla_gain = DetlaGain(i, j);
...
void KL::RecordMoves(const std::vector<std::pair<int, KL::PII>> &order, int m, std::vector<KL::PII> &moves) {
    for (int i = 0; i <= m; i++) {
...
std::vector<KL::PII> KL::kl(KL::Graph &g) {
    ...
    while (Gm > 0) {
        ...
        g.SetFree(); //set every node as free
        g.UpdateAllCosts(); //compute D(v) for each node
        while (!g.IsFixed()) { 
            //while all cells are not fixed 
            //select free node a from A and free node b from B
            //∆g = D(a) + D(b) - 2c(a, b), ∆g is maximized
            std::pair<int, std::pair<int, int>> max_g = g.MaxGain();  
            int a = max_g.second.first, b = max_g.second.second;
            DBG("max∆g = %d\n", max_g.first);
            order.emplace_back(max_g.first, PII(a, b)); //keep track of swapped cells
            g.TryMove(a, b); //actual move
            g.Fix(a);
            g.Fix(b);
            DBG("swap %c and %c\n", a + 'a', b + 'a');
            g.UpdateAllCosts(); //update D(v) for free node connected to a and b;(lazy)
            i = i + 1;
        }
        PII mp = g.BestMoves(order);//swap sequence 1...m that maximizes Gm
        Gm = mp.first;
        int m = mp.second;
        DBG("Gm = %d, m = %d\n", Gm, m + 1);
        if (Gm > 0) {
            //execute move sequences
            g.Recovery(order, i - 2, m);
